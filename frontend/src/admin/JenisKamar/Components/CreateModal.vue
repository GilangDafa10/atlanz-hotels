  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="isOpen" class="fixed inset-0 z-[9999] flex items-center justify-center px-4">
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative z-10 overflow-hidden">
          ...
          <form @submit.prevent="submitForm" class="space-y-4">
            <div>
              ...
              <input v-model="form.jenis_kasur" type="text" placeholder="Contoh: King Size" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition" required />
            </div>
            ...
            <div>
              ...
              <div class="relative">
                ...
                <input v-model.number="form.harga_permalam" type="number" placeholder="0" class="w-full border border-gray-300 rounded-lg pl-10 pr-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition" required />
              </div>
            </div>

            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi</label>
              <textarea v-model="form.deskripsi" rows="3" placeholder="Deskripsi fasilitas kamar..." class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"></textarea>
            </div>

            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Upload Gambar *</label>
              <input type="file" accept="image/*" @change="handleFileUpload" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition" required />
              
              <div v-if="previewImage" class="mt-3 relative group">
                <img :src="previewImage" class="w-full h-40 object-cover rounded-lg shadow-sm border" />
                <button type="button" @click="removeImage" class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 shadow hover:bg-red-600 transition opacity-0 group-hover:opacity-100">
