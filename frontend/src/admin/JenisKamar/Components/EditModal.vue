        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative z-10">
          ...
          <form @submit.prevent="submitEdit" class="space-y-4">
            <div>
              ...
              <input v-model="form.jenis_kasur" type="text" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none transition" required />
            </div>
            ...
            <div>
              ...
              <div class="relative">
                ...
                <input v-model.number="form.harga_permalam" type="number" class="w-full border border-gray-300 rounded-lg pl-10 pr-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none transition" required />
              </div>
            </div>

            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-1">Deskripsi</label>
              <textarea v-model="form.deskripsi" rows="3" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 outline-none transition"></textarea>
            </div>
            ...
            <div>
              ...
              <div class="mb-3">
                <img v-if="newPreview" :src="newPreview" class="w-full h-40 object-cover rounded-lg border shadow-sm" alt="New Preview" />
                <img v-else-if="existingPreview" :src="existingPreview" class="w-full h-40 object-cover rounded-lg border shadow-sm" alt="Existing" />
                <div v-else class="w-full h-40 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 border border-dashed border-gray-300">
                  Tidak ada gambar
                </div>
              </div>

              <input type="file" accept="image/*" @change="handleFileChange" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition" />
